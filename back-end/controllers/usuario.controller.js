...
const { usuario, rol, departamento, usuariodepartamento } = require('../models');
...
const registrarUsuario = async (req, res) => {
    try {
        ...
        if (!correo.endsWith(dominioPermitido)) {
            return res.status(400).json({ mensaje: 'Correo no autorizado. Debe terminar en ' + dominioPermitido });
        }
...
const asignarDepartamentos = async (req, res) => {
    try {
        ...
        if (!Array.isArray(departamentos)) {
            return res.status(400).json({ mensaje: 'Se requiere un arreglo de IDs de departamentos.' });
        }
...
const loginUsuario = async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      departamentos: user.departamentos.map(dep => ({ id: dep.id, nombre: dep.nombre }))
    });
