...
async function llenarSidebarDepartamentos() {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/api/departamentos/usuario/${usuarioActual.id}`
    );
...
function renderizarProyectos(proyectos, idUsuario) {
  ...
  proyectos.forEach((proyecto) => {
    ...
    const listaEncargados =
      encargados.map((enc) => `${enc.nombre} ${enc.apellidos}`).join(', ') ||
      'Sin asignar';
...
export { filtrarProyectosPorDepartamento, llenarSidebarDepartamentos };
